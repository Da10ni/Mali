    ...
    <div className="mt-2">
      ...
      <div className="grid grid-cols-7 gap-1 text-center">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day, index) => (
          <div key={index} className={`text-xs font-medium  mb-1 ${textColor}`}>{day}</div>
        ))}
      </div>
      <div className={`grid grid-cols-7 gap-1 text-center ${textColor}`}>
        {calendarDays.map((day, idx) => {
          ...
          let cellClass = "py-1 text-xs rounded-full ";
          
          if (!available || isBlocked) {
            cellClass += "bg-gray-200 text-gray-400 cursor-not-allowed";
          } else if (hasBooked) {
            cellClass += "bg-orange-100 text-black  cursor-pointer hover:bg-orange-200";
          } else {
